...
/**
 * A software-only Proportional Integral Differential (PID) controller with optional support for
 * feed forward. The source of
 * inputs and the output to which results are written are both supplied as functions upon
 * construction.
 * <p>
 * There are two ways to use this controller:
 * <ol>
 * <li>Manually - a controller instance is used within or as a subsystem, but it is only run when it
 * is used by commands. The
 * controller's {@link #withTarget(double) target}, {@link #withTolerance(double) tolerance}, and
 * other settings are typically
 * modified , typically with commands.
 * <li>With commands - a controller instance is used within or as a subsystem, and one or more
 * command classes are created to
 * use this by setting the {@link #withTarget(double) setpoint} (and optionally the gains, input and
 * output ranges, and
 * tolerance) and calling {@link #computeOutput()}; or</li>
 * <li>Continuous execution - a controller instance is registered with an {@link Executor}
 * (typically Strongback's
 * {@link Strongback#executor() built-in executor}) to continuously execute and generate the output
 * based upon the input and
 * current setpoint. The setpoint, gains, input and output ranges, and tolerance can all be changed
 * at any time, including.</li>
 * </ol>
 * <h2>Recording data</h2> It is possible to register a PIDController with a {@link DataRecordable}
 * to record separate channels
 * for this controller's instantaneous values used. To record the input, output, setpoint, and error
 * values on separate
 ...
 * <h2>Profiles</h2>
 * <p>
 * Each {@link SoftwarePIDController} instance is able to have 1 or more independent sets of PID
 * gains, called <em>profiles</em>
 * . One of these profiles is always in use, and by default it is the "default" profile. Additional
 * profiles can be
 * {@link #withProfile(int, double, double, double, double) defined}, and then while in operation
...
public class SoftwarePIDController implements PIDController {
    ...
    /**
     * Create a new PID+FF controller that uses the supplied source for inputs and sends outputs to
     * the supplied consumer.
     * Before using, be sure to set the {@link #withGains(double, double, double, double) PID and
     * feed forward gains}, the
     * {@link #withInputRange(double, double) input range}, {@link #withOutputRange(double, double)
     * output range},
     * {@link #withTarget(double) setpoint}, {@link #withTolerance(double) tolerance}, and whether
     * the inputs are
     ...
     */
    public SoftwarePIDController(Supplier<SourceType> sourceType, DoubleSupplier source,
     ...
     * @param output the output to which the calculated output is to be send; may not be null
     */
    public SoftwarePIDController(SourceType sourceType, DoubleSupplier source,
            DoubleConsumer output) {
        if (sourceType == null)
            throw new IllegalArgumentException("The source type may not be null");
        if (source == null)
    ...
    /**
     * Get the {@link Executable} instance that will continuously {@link #computeOutput() execute}
     * this controller to read
     * inputs from the source and generate outputs to reach the {@link #withTarget(double)
     * setpoint}.
     * <p>
     * If this is used, then this same controller should <em>never</em> be used with commands. This
     * is not checked, so robot
     * programs are responsible for ensuring this does not happen.
     *
     * @return the {@link Executable} object that can be registered with an {@link Executor}
     *         (typically Strongback's
     *         {@link Strongback#executor() central executor}); never null and always the same
    ...
    public SoftwarePIDController useProfile(int profile) {
        ...
        if (gains == null)
            throw new IllegalArgumentException("No profile '" + profile + "' exists");
        synchronized (this) {
            this.gains = gains;
     ...
     *
     * @param continuous <code>true</code> if the input values are continuous values, and if the
     *        controller can take into
     *        account the error between the actual source and the input values; or
     *        <code>false</code> if the controller should
    ...
    public SoftwarePIDController withInputRange(double minimumInput, double maximumInput) {
        ...
        Target target = this.target.withInputRange(minimumInput, maximumInput); // recalculates
                                                                                // setpoint within
                                                                                // range
        this.target = target;
    ...
    /**
     * Sets the target value for this controller. The input range should be set prior to this call,
     * and the setpoint value
     * should be within the {@link #withInputRange(double, double) input range} values; otherwise,
     * the setpoint will be capped
     ...
     *
     * @param tolerance the maximum absolute error which is tolerable in the units of the input
     *        object
     * @return this object so that methods can be chained; never null
     * @see #withInputRange(double, double)
     */
            ...
            switch (sourceType) {
                case RATE:
                    ...
                    if (gains.p != 0) {
                        ...
                        result = (gains.p * totalError) + (gains.d * error)
                ...
                case DISTANCE:
                    ...
                    result = (gains.p * error) + (gains.i * totalError)
                            + (gains.d * (error - prevError))
                            + (target.setpoint * gains.feedForward);
                    break;
            }
    ...
    /**
     * Get a {@link DataRecordable recordable} object that can be used to
     * {@link DataRecorder#register(String, DataRecordable)
    ...
    protected static final class Target {
        ...
        public Target(double setpoint, double minInput, double maxInput, double tolerance,
                boolean continuous, double minOutput,
                double maxOutput) {
        ...
        public Target withInputRange(double minInput, double maxInput) {
            return new Target(setpoint, minInput, maxInput, tolerance, continuous, minOutput,
                    maxOutput);
        }
