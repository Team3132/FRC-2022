...
/**
 * A threadsafe {@link DataRecorder} that allows for switches, motors and other functions to be
 * registered, and then to
...
final class DataRecorderDriver implements Executable {
    ...
    DataRecorderDriver(DataRecorderChannels channels,
            Function<Iterable<DataRecorderChannel>, DataWriter> writerFactory) {
    ...
    public synchronized void start() {
        writer.getAndUpdate(
                (existing) -> existing == NULL_WRITER ? writerFactory.apply(channels) : existing);
    }
    ...
    public synchronized void stop() {
        // We will always replace the existing data writer with NULL_WRITER, but only after we do
        // this do we close the
        // writer. These steps are done in a very strict and ordered manner to ensure the
        // non-NULL_WRITER is always closed.
