...
/**
 * A relay is a device that can be turned on and off. Note that a relay has one of 5 possible
 * states:
 * <ol>
 * <li>ON - the relay is in the "on" position;</li>
 * <li>OFF - the relay is in the "off" position;</li>
 * <li>SWITCHING_ON - the relay was in the "off" position but has been changed and is not yet in the
 * "on" position;</li>
 * <li>SWITCHING_OFF - the relay was in the "on" position but has been changed and is not yet in the
 * "off" position; and</li>
 * <li>UNKNOWN - the relay position is not known</li>
 * </ol>
 * <p>
 * Not all Relay implementations use all relay states. Very simple relays that have no delay will
 * simply only use
 * {@link State#ON} and {@link State#OFF}, while relays that have some delay might also use
 * {@link State#SWITCHING_ON} and
 * {@link State#SWITCHING_OFF}. Those relay implementations that may not know their position upon
 * startup may start out in the
 * UNKNOWN state.
 *
...
public interface Relay {
    ...
    static enum State {
        /**
         * The relay is presently switching into the "ON" state but has not yet completed the
         * change.
         */
        SWITCHING_ON,
        ...
        ON,
        /**
         * The relay is presently switching into the "OFF" state but has not yet completed the
         * change.
         */
        SWITCHING_OFF,
     ...
     * {@code state() == State.SWITCHING_ON}.
     *
     * @return {@code true} if this relay is in the process of switching from off to on; or
     ...
     * {@code state() == State.SWITCHING_OFF}.
     *
     * @return {@code true} if this relay is in the process of switching from on to off; or
     *         {@code false} otherwise
     */
    ...
    /**
     * Obtain a relay that remains in one fixed state, regardless of any calls to {@link #on()} or
     * {@link #off()}.
     *
    ...
    /**
     * Obtain a relay that instantaneously switches from one state to another using the given
     * functions.
     *
     * @param switcher the function that switches the state, where <code>true</code> represents
     *        {@link State#ON} and
     *        <code>false</code> represents {@link State#OFF}; may not be null
     * @param onState the function that returns <code>true</code> if the current state is
     *        {@link State#ON}, or
