...
/**
 * A sensor that determines the positional heading, or angular displacement, in degrees. A compass
 * is an {@link AngleSensor}
 * with an additional method to determine {@link #getHeading() heading}. It also can be
 * {@link #zero() zeroed} to return angles
 * and heading relative to another. Negative values are assumed to be counter-clockwise and positive
...
public interface Compass extends AngleSensor {
    ...
    /**
     * Compute the change in heading between the {@link #getHeading() current heading} and the
     * target heading, using the given
     * tolerance for the difference. The result is the angle that this sensor must rotate to reach
     * the target heading, which may
     * be positive or negative.
     *
     * @param targetHeading the target heading
     * @param tolerance the allowed tolerance in degrees between the two headings
     * @return the angular displacement required for this sensor to reach the target heading, or
     *         0.0d if the two headings are
