...
public interface Clock {
     ...
     */
    default public void sleepSeconds(double seconds) throws InterruptedException {
     ...
     * @throws InterruptedException
     */
    default public void sleepMilliseconds(double milliseconds) throws InterruptedException {
     ...
     * @throws InterruptedException
     */
    default public void sleepMicroseconds(long micros) throws InterruptedException {
    ...
    /**
     * Create a new time system that uses the FPGA clock. At this time, the precision of the
     * resulting clock has not been
     * verified or tested.
     *
    ...
    public static Clock fpga() {
        try {
            ...
            // If we're here, then the method did not throw an exception and there is FPGA hardware
            // on this platform ...
    ...
    /**
     * Create a new time system that uses the FPGA clock if it is available, or the {@link #system()
     * JRE's time system} if not.
     *
    ...
    /**
     * Create a new time system that uses the JRE's System clock. The performance and precision of
     * the JVM clock depends upon
     * the accuracy and precision of the underlying platform's mechanism for returning absolute and
     * relative time. Absolute time
     * can be obtained via {@link #currentTimeInMillis()}, although precision on some platforms may
     * be as low as 10 or 20
     * milliseconds. Relative time that can be compared within a single process can be obtained via
     * {@link #currentTimeInNanos()} and {@link #currentTimeInMicros()}, although again the
     * precision may be at best around 10
     * or 20 microseconds.
     * <p>
     * Additionally, care must be taken when using relative time on multi-core systems. Because
     * relative time often uses the
     * CPU's clock, relative time obtained from different CPUs might not be comparable. This is
     * possible even if using a single
     * thread, since the same thread might be scheduled on a different CPU after switching thread
     * contexts.
     *
