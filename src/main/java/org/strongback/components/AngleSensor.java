...
/**
 * A sensor that returns an angle. The sensor's can be {@link #zero() zeroed} to reset the angle at
 * which the sensor considers 0
 * degrees.
 */
@ThreadSafe
@FunctionalInterface
public interface AngleSensor extends Zeroable {
    ...
    /**
     * Compute the change in angle between the {@link #getAngle() current angle} and the target
     * angle, using the given tolerance
     * for the difference. The result is the angle that this sensor must rotate to reach the target
     * angle, and may be positive
     * or negative
     *
     * @param targetAngle the target angle
     * @param tolerance the allowed tolerance in degrees between the two angles
     * @return the positive or negative angular displacement required for this sensor to reach the
     *         target angle, or 0.0d if the
