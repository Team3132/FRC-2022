...
public interface ContinuousRange {
    ...
    /**
     * Create a new range that reads zero if the absolute value of this instance is less than the
     * theshold
     *
    ...
    /**
     * Create a new range that squares the values whilst preserving their signs
     * 
     * @return new squared integer, with unchanged sign
     */
    default public ContinuousRange squarePreservingSign() {
        return () -> this.read() * this.read() * Math.signum(this.read());
    };
}
