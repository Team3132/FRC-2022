...
public interface Fuse extends Switch {
    /**
     * Trigger the fuse. Once this method is called, the {@link #isTriggered()} switches but then
     * will never change until it is
     * {@link #reset()}.
     *
    ...
    /**
     * Create a simple fuse that is manually {@link #trigger() triggered} and manually
     * {@link #reset()}.
     *
     ...
     * {@link #reset()}.
     *
     * @param whenTriggered the function that is to be called when the fuse is triggered; may be
     *        null
     * @return the fuse; never null
     */
    ...
    /**
     * Create a fuse that can be manually {@link #reset()} but that will automatically reset after
     * the given delay.
     *
     * @param delay the time after the fuse is triggered that it should automatically reset; must be
     *        positive
    ...
    public static Fuse autoResetting(long delay, TimeUnit unit, Clock clock,
        ...
        return new Fuse() {
            ...
            public synchronized boolean isTriggered() {
                if (triggered && theClock.currentTimeInMillis() > resetTime)
                    triggered = false;
                return triggered;
            }
