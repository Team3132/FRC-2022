...
/**
 * A simple counter that is safe to be concurrently used (both {@link #get()} and
 * {@link #increment()}) in one or more threads.
 *
...
public interface Counter extends Zeroable {
    ...
    /**
     * Create a new counter that will monotonically increment by 1 and upon reaching the
     * {@link Integer#MAX_VALUE maximum
    ...
    /**
     * Create a new counter that will increment by 1 and that upon reaching the supplied maximum
     * value will upon the next
    ...
    /**
     * Create a new counter that will increment by the specified amount and that upon reaching the
     * supplied maximum value will
     * upon the next {@link #increment()} call automatically reset to the given initial value.
     *
     * @param initial the initial value for the counter; must be 0 or positive
     * @param increment the value by which the counter should be incremented when
     *        {@link #increment()} is called; must be
     *        positive
     * @param maximum the maximum value the counter can achieve; must be positive and greater than
     *        {@code initial}
     * @return the new counter; never null
     * @throws IllegalArgumentException if {@code initial} or {@code resetValue} are negative, or if
     *         {@code maximum} is less
    ...
    public static Counter circular(int initial, int increment, int maximum) {
        if (initial < 0)
            throw new IllegalArgumentException("The intial value must be non-negative");
        if (maximum <= initial)
            throw new IllegalArgumentException(
                    "The maximum value must be greater than the initial value");
        if (increment < 1)
            throw new IllegalArgumentException("The increment value must be positive");
        return new Counter() {
            ...
            public Counter increment() {
                ...
                // ...
                value.getAndUpdate(
                        (currentValue) -> currentValue < maximum ? currentValue + increment
                                : initial);
                return this;
            }
