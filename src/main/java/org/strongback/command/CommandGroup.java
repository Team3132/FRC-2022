...
/**
 * A {@link CommandGroup} is a series of {@link Command}s executed in sequence or in parallel. A
 * command group is created by
 * extending {@link CommandGroup}, and then specifying in the constructor of the subclass whether
 * the {@link Command}s should be
 * executed {@link #sequentially(Command...) sequential} or {@link #simultaneously(Command...)
 * simultaneously}. The commands
 * passed into these methods can be any {@link Command} subclass, {@link CommandGroup} subclass, or
 ...
 * {@link #sequentially(Command...)} and the list of {@link Command} instances, in order. The whole
 * block finishes only when the
 * last {@link Command} is done.
 *
 * <p>
 * Commands executed at the same time (in parallel) are <em>simultaneous</em>, and are defined with
 * a call to
 * {@link #simultaneously(Command...)} and a list of the {@link Command} instances. Order is not
 * important, as they are all are
 * executed at the same time. The whole block finishes only when all {@link Command}s have
 * completed.
 *
 * <p>
 * When a command is to be executed completely independently of the command group, then that command
 * (or group of commands)
 * can be <em>forked</em>. A command (or group) is forked by calling {@link #fork(Command)} with the
 * one {@link Command}, a
 * {@link CommandGroup} subclass, or a group created with {@link #sequentially(Command...)} or
 * {@link #simultaneously(Command...)}. For the purposes of {@link #sequentially(Command...)} the
 * block finishes instantly.
 *
 * <h2>Sequential example</h2>
 * <p>
 * The following code shows a complete CommandGroup subclass that, when executed, first executes a
 * {@code CommandA}, then a
 ...
 * <p>
 * The following code shows a complete CommandGroup subclass that, when executed, executes three
 * commands (a {@code CommandA}, a
 ...
 * <p>
 * The following code shows a complete CommandGroup subclass that, when executed, executes two
 * commands (a {@code CommandA} and
 * a {@code CommandB}) at the same time and after both are finished a third command,
 * {@code CommandC}:
 *
 * <pre>
 ...
 * </pre>
 *
 * The {@code MyMixedCommands} will complete when the last of the three commands ({@code CommandC})
 * completes.
 *
 * <h2>Complex example</h2>
 * <p>
 * The following code shows a complete but more complex CommandGroup subclass that, when executed,
 * first executes
 * {@code CommandA}, then executes {@code CommandB}, then forks off {@code CommandC} and immediately
 * executes {@code CommandD},
 ...
 * </pre>
 *
 * The {@code MyForkCommands} will complete when the last of the two final commands
 * ({@code CommandE} or {@code CommandF})
...
public class CommandGroup extends Command {
    ...
    /**
     * Creates a single {@link CommandGroup} that executes several {@link Command}s in sequential
     * order.
     *
    ...
    /**
     * Create a new command group. Typically, subclass constructors call this constructor (perhaps
     * implicitly) and then add one
     * or more commands using {@link #fork(Command)}, {@link #sequentially(Command...)}, and
     * {@link #simultaneously(Command...)}
     * .
     */
    ...
    /**
     * Wraps several {@link Commands}s in a single {@link CommandGroup} that executes them
     * simultaneously.
     *
    ...
    /**
     * Add a {@link Command} that executes independently of any other {@link Command}s in this
     * group.
     *
