...
final class CommandRunner {
    ...
    private static final Context DEFAULT_CONTEXT =
            Context.with(NO_OP_LISTENER, Strongback.logger());
     ...
     * @return {@code true} if this {@link CommandRunner} is ready to be terminated; {@code false}
     *         otherwise
     */
    boolean step(long timeInMillis) {
        ...
        if (state == CommandState.UNINITIALIZED) {
            try {
                ...
                logger().error(t, "Error while initializing " + command.getClass().getName()
                        + " command: " + command);
                state = CommandState.INTERUPTED;
            }
        }
        ...
        if (state == CommandState.RUNNING) {
            try {
                ...
                logger().error(t, "Error while executing " + command.getClass().getName()
                        + " command: " + command);
                state = CommandState.INTERUPTED;
            }
        }
        ...
        if (state == CommandState.INTERUPTED) {
            try {
                ...
                logger().error(t, "Error while interrupting " + command.getClass().getName()
                        + " command: " + command);
            }
        ...
        if (state == CommandState.FINISHED) {
            ...
            try {
                ...
                logger().error(t, "Error while ending " + command.getClass().getName()
                        + " command: " + command);
            }
            state = CommandState.FINALIZED;
            listener().record(command, state);
     ...
     */
    public void cancel() {
        // We want to change the state to INTERRUPTED, but for thread-safety we can't actually
        // change the `state` field here
        // since it is actively used within the `step(...)` method. So instead, we'll set the
        // `cancelled` flag (this is the
    ...
    public String toString() {
        ...
        if (children != null) {
            return next == null ? Arrays.toString(children)
                    : Arrays.toString(children) + " -> " + next;
        }
