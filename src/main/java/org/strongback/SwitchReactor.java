...
/**
 * A class that allows other components to react to the state of switches. For example, the reactor
 * can be used to submit a
 * command whenever a Gamepad's left trigger button is pressed and submit a different command when
 * the same trigger is released:
 *
 * <pre>
 *   Gamepad gamepad = ...
 ...
 * </pre>
 * <p>
 * The reactor is threadsafe, meaning functions can be registered even while the reactor is
 * forwarding state and state
...
public interface SwitchReactor {
     ...
     * @param commandSupplier the supplier of the command to submit; may not be null but may return
     *        a null command
     */
    ...
    /**
     * Register a {@link Runnable} function that is to be called the moment when the specified
    ...
    /**
     * Register a {@link Runnable} function to be called repeatedly whenever the specified
     * {@link Switch} is in the triggered
     * state.
     *
     * @param swtch the {@link Switch}
