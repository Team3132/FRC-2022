...
/**
 * A threadsafe {@link SwitchReactor} implementation that relies upon being periodically
 * {@link Executable#execute(long)
 * executed}. This class is carefully written to ensure that all functions are registered atomically
 * even while
...
final class AsyncSwitchReactor implements Executable, SwitchReactor {
    ...
    /**
     * A container class for all listener functions associated with a specific {@link Switch}. The
     * class is threadsafe to allow
     * for new listener functions to be added while the existing functions are called based upon the
     * switch's current state.
     * <p>
     * To achieve efficient and lock-free concurrent operations, each of the functions for a
     * specific Switch state or transition
     * are maintained in a simple linked-list structure (see {@link Listener}). Each immutable
     * Listener is created to hold one
     * function and an optional "next" listener. To add a new function, a new Listener object is
     * created with the function and
     * the current Listener object for that state, and the Container's reference to that state's
     * listeners is updated with the
     * new Listener object. In essence, new functions are added to the front of the linked list
     * without using any locking.
     * <p>
    ...
    private static final class Container {
        ...
        private void notifyAtomicallyWhen(BooleanSupplier criteria,
                AtomicReference<Listener> listenerRef) {
