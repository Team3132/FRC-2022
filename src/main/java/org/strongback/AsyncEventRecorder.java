...
/**
 * A thread-safe and lock-free {@link EventRecorder} implementation that records all events to a
 * thread-safe queue and then when
 * {@link #execute(long) executed} writes all enqueued events in the same order as received. The
 * {@link AsyncEventRecorder} is
 * {@link Executable}, and is designed to be {@link Executor#register(Executable) registered} with
 * an {@link Executor} to
...
final class AsyncEventRecorder implements EventRecorder {
    ...
    protected static interface EventWriter extends AutoCloseable {
        ...
        /**
         * Record an event with the specified {@link EventType#typeNumber() event type number} and
         * String value. Before this
         * method is called, {@link #recordEventType(long, EventType)} will have been called for the
         * given type of event
         *
         * @param timeInMillis the time (in milliseconds) of the event
