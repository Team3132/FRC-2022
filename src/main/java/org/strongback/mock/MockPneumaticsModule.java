...
/**
 * A mock implementation of the {@link PneumaticsModule} that allows the caller to control the
 * {@link #lowPressureSwitch()}.
 * When the module's {@link #automaticMode()} is enabled, then triggering the
 * {@link #lowPressureSwitch() low pressure switch}
 * will start the compressor, and {@link MockSwitchImplementation#setNotTriggered() un-triggering}
 * the {@link #lowPressureSwitch() low
...
public class MockPneumaticsModule implements PneumaticsModule {
    ...
    private final MockSwitchImplementation running =
            new MockSwitchImplementation().setNotTriggered();
    private final MockSwitchImplementation lowPressure = new MockSwitchImplementation() {
        @Override
        public MockSwitchImplementation setTriggered(boolean triggered) {
            super.setTriggered(triggered);
            if (automatic.isOn()) {
                // The caller is manually triggering the switch, but this affects whether compressor
                // runs ...
    ...
    @Override
    public MockSwitchImplementation lowPressureSwitch() {
        return lowPressure;
    }
    ...
    private void triggerFault(MockSwitchImplementation stickySwitch) {
    ...
    protected static class StickyFaults implements Faults {
        private final MockSwitchImplementation currentTooHigh =
                new MockSwitchImplementation().setNotTriggered();
        private final MockSwitchImplementation notConnected =
                new MockSwitchImplementation().setNotTriggered();
        private final MockSwitchImplementation shorted =
                new MockSwitchImplementation().setNotTriggered();
    ...
    @Override
    public Solenoid singleSolenoid(int extendChannel, double timeIn, double timeOut) {
        return new MockSingleSolenoid(timeOut, timeIn);
    }

    @Override
    public Solenoid doubleSolenoid(int extendChannel, int retractChannel, double timeIn,
            double timeOut) {
        return new MockDoubleSolenoid(timeOut, timeIn);
    }
}
