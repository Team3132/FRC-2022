...
/**
 * A {@link Gyroscope} implementation useful for testing, where the angle and angular rate can be
 * explicitly set in the test
...
public class MockGyroscope extends MockZeroable implements Gyroscope {
    ...
    private final MockUpdateTimer timer = new MockUpdateTimer();
    ...
    public MockGyroscope setRate(double angularVelocity) {
        this.rate = angularVelocity;
        System.out.println("Setting Angular Velocity to: " + this.rate);
        return this;
    }

    public void update() {
        // update the position depending on the rate.
        setAngle(getAngle() + rate * timer.diff());
    }
}
