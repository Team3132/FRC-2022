...
/**
 * Control logic for a tank/skid-style drive system. This controller provides
 * {@link #arcade(double, double) arcade-style},
 * {@link #tank(double, double) tank-style}, and {@link #cheesy(double, double, boolean)
 * cheesy-style} inputs.
 * <p>
 * This drive implements {@link Requirable} so that {@link Command}s can use it directly when
...
public class TankDrive implements Requirable {
    ...
    public static final DoubleToDoubleFunction DEFAULT_SPEED_LIMITER =
    ...
    /**
     * Creates a new DriveSystem subsystem that uses the supplied drive train and no shifter. The
     * voltage send to the drive
     * train is limited to [-1.0,1.0] with the exception of the (-0.02,0.02) deadband around the
     * zero point.
     *
     ...
     * The voltage send to the
     * drive train is limited to [-1.0,1.0] with the exception of the (-0.02,0.02) deadband around
     * the zero point.
     *
     * @param left the left motor on the drive train for the robot; may not be null
     * @param right the right motor on the drive train for the robot; may not be null
     * @param shifter the optional shifter used to put the transmission into high gear; may be null
     *        if there is no shifter
     */
     ...
     * @param right the right motor on the drive train for the robot; may not be null
     * @param shifter the optional shifter used to put the transmission into high gear; may be null
     * @param speedLimiter the function that limits the speed sent to the drive train; if null, then
     *        a default clamping function
     *        is used to limit to the range [-1.0,1.0] with the exception of the (-0.02,0.02)
     *        deadband around the zero point
     */
    ...
    /**
     * Shift the transmission into high gear. This method does nothing if the drive train has no
    ...
    /**
     * Shift the transmission into low gear. This method does nothing if the drive train has no
     * transmission shifter.
     */
    public void lowGear() {
        this.highGear.off();
    ...
    public void tank(double leftSpeed, double rightSpeed, boolean squaredInputs) {
        // square the inputs (while preserving the sign) to increase fine control while permitting
        // full power
    ...
    /**
     * Used in the {@link #cheesy(double, double, boolean) cheesy drive} logic, this function
     * dampens the supplied input at
    ...
    /**
     * Provide "cheesy drive" steering using a steering wheel and throttle. This function lets you
     * directly provide joystick
     * values from any source.
     * <p>
     * "Cheesy Drive" simply means that the "turning" stick controls the curvature of the robot's
     * path rather than its rate of
     * heading change. This helps make the robot more controllable at high speeds. Also handles the
     * robot's quick turn
     * functionality - "quick turn" overrides constant-curvature turning for turn-in-place
     * maneuvers.
     * <p>
     * This is equivalent to calling
     * {@code cheesy(throttle, wheel, isQuickTurn, CheesyAlgorithm.SIMPLE)}.
     *
     * @param throttle the value of the throttle, which is generally in the range -1 to 1 inclusive
     *        but limited by the speed
     *        limiter provided in the constructor
     * @param wheel the value of the steering wheel, which is generally in the range -1 to 1
     *        inclusive but limited by the speed
     *        limiter provided in the constructor and where negative values turn right and positive
     *        values turn left
     * @param isQuickTurn true if the quick-turn button is pressed
     * @see <a href=
     *      "https://github.com/Team254/FRC-2016-Public/blob/master/src/com/team254/frc2016/CheesyDriveHelper.java">Team
     ...
     * <li><b>inertia</b> - an earlier algorithm that uses inertia terms to non-linearly smooth the
     * outputs, from
     * <a href=
     * "https://github.com/Team254/FRC-2016-Public/blob/master/src/com/team254/frc2016/CheesyDriveHelper.java">Team
     * 254
     ...
     * @param wheel the value of the steering wheel; must be -1 to 1, inclusive Negative values turn
     *        right; positive values turn
     *        left.
     * @param isQuickTurn true if the quick-turn button is pressed
     * @see <a href=
     *      "https://github.com/Team254/FRC-2014/blob/master/src/com/team254/frc2014/CheesyDriveHelper.java">Team
     *      254
        ...
        if (isQuickTurn) {
            if (Math.abs(linearPower) < 0.2) {
                ...
                quickStopAccumulator = (1 - alpha) * quickStopAccumulator
                        + alpha * Values.symmetricLimit(0.0, wheel, 1.0) * 5;
            }
            overPower = 1.0;
            if (isHighGear) {
                sensitivity = 1.0;
    ...
    /**
     * Provide a simpler "cheesy drive" steering using a steering wheel and throttle. This function
     * lets you directly provide
     * joystick values from any source.
     * <p>
     * "Cheesy Drive" simply means that the "turning" stick controls the curvature of the robot's
     * path rather than its rate of
    ...
    private void cheesy2016(double throttle, double wheel, boolean isQuickTurn) {
        ...
        if (isQuickTurn) {
            if (Math.abs(throttle) < DEFAULT_MINIMUM_SPEED) {
                double alpha = 0.1;
                quickStopAccumulator = (1 - alpha) * quickStopAccumulator
                        + alpha * Values.symmetricLimit(0.0, wheel, 1.0) * 2;
            }
            overPower = 1.0;
            angularPower = wheel;
        } else {
