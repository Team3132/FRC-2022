 ...
 * <p>
 * There are two kinds of data that Strongback supports in its recording: events and data. Data are
 * essentially continuous, and
 * Stronback's {@link DataRecorder} is used to capture at regular time steps values for all
 * registered continuous channels.
 * Events, on the other hand, are far less frequent occurrences of some type of action; recording
 * them as a channel is
 * inefficient (since there is no value at most time steps) and often makes little sense, so instead
 * the {@link EventRecorder}
 * allows components to record these spurious and infrequent events. Both the data and event records
 * can then be combined to
 * view a complete timeline with all available information.
 * <p>
 * Strongback's command scheduler can be {@link Strongback#configure() configured} to automatically
 * record the state transitions
 * of {@link Command}s as they are executed. Of course, custom robot code can also record any other
 * kinds of events.
 * <p>
 * Implementations of this class are expected to be thread-safe so that any of the
 * {@link #record(String, String) record(...)}
