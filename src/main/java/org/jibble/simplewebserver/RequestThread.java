...
public class RequestThread extends Thread {
    ...
    private static void sendHeader(BufferedOutputStream out, int code, String contentType,
    ...
    private static void sendError(BufferedOutputStream out, int code, String message)
            throws IOException {
        ...
        try {
            ...
            if (!file.toString().startsWith(_rootDir.toString())) {
                ...
                out.write(("<html><head><title>" + title
                        + "</title></head><body><table><h3>3132 - Index of " + path + "</h3><p>\n")
                                .getBytes());

                out.write(("<tr>" + "<td>" + "File Name" + "</td>" + "<td>" + "Size" + "</td>"
                        + "<td>" + "Type" + "</td>").getBytes()); // Added by @RKouchoo
                ...
                for (int i = 0; i < files.length; i++) {
                    ...
                    out.write(("<tr>" + "<td>" + "<a href=\"" + path + filename + "\">" + filename
                            + "</a> " + "</td>" + "<td style=\"float: right\">"
                            + file.length() / 1000000 + " MB" + "<td>"
                            + "<td style=\"float: right\">" + description + "<td>" + "<br>\n"
                            + "</tr>").getBytes()); // @RKouchoo added file.length()
                }
                out.write(("</p><hr><p>" + "Team 3132 - " + SimpleWebServer.VERSION
                        + "</p></body></table><html>").getBytes());
            } else {
                reader = new BufferedInputStream(new FileInputStream(file));

                String contentType =
                        (String) SimpleWebServer.MIME_TYPES.get(SimpleWebServer.getExtension(file));
                if (contentType == null) {
