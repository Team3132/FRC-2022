 ...
 *
 */
public class GyroscopeTest extends AngleSensorTest {

    private static final double[][] POSITIVE_ANGLES = {{0.0, 0.0}, {180.0, 180.0}, {359.0, 359.0},
            {360.0, 0.0},
            {360.001, 0.001}, {361.0, 1.0}, {360.0 * 2, 0.0}, {360.0 * 3, 0.0}, {360.0 * 4, 0.0}};

    private static final double[][] NEGATIVE_ANGLES =
            {{-1.0, 359}, {-90.0, 270}, {-180, 180}, {-270, 90},
                    {-360, 0}, {-361, 359}, {-360 * 2, 0}};

    private static final double NOMINAL_TOLERANCE = 0.0001;
    ...
    protected static void assertProperHeading(double angle, double expectedHeading) {
        setValue(angle);
        assertEquals(sensor.getAngle(), angle, NOMINAL_TOLERANCE);
        assertEquals(sensor.getHeading(), expectedHeading, NOMINAL_TOLERANCE);
    }
    ...
    private static Gyroscope sensor =
            Gyroscope.create(GyroscopeTest::getValue, GyroscopeTest::getRate);
    ...
    @Test
    public void shouldNotAllowZeroingToAffectRate() {
        setValue(45);
        setRate(20);
        assertEquals(sensor.getRate(), 20, NOMINAL_TOLERANCE);
        sensor.zero();
        assertAngle(45, 0);
        assertEquals(sensor.getRate(), 20, NOMINAL_TOLERANCE);
    }
