...
/**
 * An {@link EventWriter} that records what it sees and allows tests to verify the order and values
 * of events.
 *
...
public class AccumulatingEventWriter implements EventWriter {
    ...
    public void recordEvent(long timeInMillis, int eventType, String value) {
        assertTrue(timeInMillis > 0);
        assertTrue(eventTypes.contains(eventType));
    ...
    public void assertMatch(long time, String eventType, String value) {
        assertTrue(events.pop().matches(time, eventTypesByName.get(eventType).typeNumber(), value));
    }
