...
public abstract class AbstractDoubleMockTest {
    ...
    protected static double[] TEST_VALUES =
            {0.01, 1.0, 0.0000001, 2.000001, 5.5, 0.99999, -1.0, -0.01, -0.000001, -2.00001,
                    -5.5};
    protected static double TEST_VALUE = TEST_VALUES[0];
    protected static double NEAR_ZERO = 0.0000000000000001d;
    protected static double NOMINAL_TOLERANCE = 0.00001d;
    ...
    /**
     * Initialize the test with the mock class's setter and getter methods. This should be called
    ...
    /**
     * Initialize the test with the {@link Zeroable} mock class's setter and getter methods. This
     * should be called within the
     * {@link Before} method of the subclass.
     *
     * @param setter the method that sets the double value; may not be null
     * @param getter the method that gets the double value; may not be null
     * @param zeroable the {@link Zeroable} reference; may be null if the component is not
     *        {@link Zeroable}
     */
    ...
    public void shouldGetLastSetValue() {
        ...
        assertEquals(getter.getAsDouble(), lastValue, NOMINAL_TOLERANCE);
    }
    ...
    public void shouldZeroWhenValueIsZero() {
        ...
        setter.accept(0.0);
        zeroable.zero();
        assertEquals(getter.getAsDouble(), 0.0, NEAR_ZERO);
    }
    ...
    protected void assertSetAndGet(double value, double tolerance) {
        setter.accept(value);
        double actual = getter.getAsDouble();
        assertEquals(actual, value, tolerance);
    }
    ...
    protected void assertSetAndZeroAndGet(double value) {
        ...
        double actual = getter.getAsDouble();
        assertEquals(actual, 0.0, NOMINAL_TOLERANCE);
        ...
        for (int i = 2; i != 6; ++i) {
            ...
            assertEquals(actual, nonZeroed - value, NOMINAL_TOLERANCE);
        }
